...
void mostrarauto(auto *x)
{
	...
	for(n=1;n<NR;n++)
	{
		if(x[n].estado!==0) /* Como todos os carros t�m um numero o zero � um carro inexistente */
		{
			printf("NUMERO=%d\nMARCA=%c\nMODELO=&c\nCOR=%c\nCILINDRADA=%d\nANO_DE_AQUIS=%d\nMATRIC=%c\nVALOR_DIA=%c\nCOMBUSTIVEL=%f\nKMS=%f\nPRECO_DE_AQUIS=%f\n\n",
...
int inserirauto(auto *x)
 {
	...
	long int inser;
	system ("cls");
	printf("Qual o autom�vel que pretende inserir? ");
	scanf("%d",&inser);
	for(n=1;n<NR;n++)
	{
		if(x[n].estado!=1)
...
void main(void) /* Inicio do programa */ 
{
  ...
  do
  {
   ...
   scanf (menu_inicial);
   		...
   		{	
   			cls
   			printf ("menu do autom�vel");
   			printf ("1- listar autom�vel");
   			printf ("2- Novo autom�vel");
   			printf ("3� Alterar autom�vel");
   			printf ("4- Apagar autom�vel");
   			printf ("5- Voltar ao menu inicial"); 
   			scanf (menu_auto);
   			lerauto(car); break;
   				while menu_auto=1
